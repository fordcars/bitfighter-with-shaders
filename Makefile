...
CFLAGS	+=	$(INCLUDE) -Iinclude -D__3DS__ -DBF_PLATFORM_3DS -DBF_NO_AUDIO -Dlinux -DTNL_NO_THREADS -DLTC_SOURCE -DLTC_NO_ROLC -DENDIAN_NEUTRAL -DTNL_ENABLE_ASSERTS
...
# To build LuaJIT
# make BUILDMODE=static     TARGET_FLAGS="-march=armv6k -mtune=mpcore -mfloat-abi=hard -mtp=soft" TARGET_T=libluajit.a     CFLAGS="" LDFLAGS="" LIBS="" HOST_CFLAGS="" HOST_LIBS="" HOST_LDFLAGS=""     HOST_CC="gcc -m32" XCFLAGS="-DLUAJIT_USE_SYSMALLOC" CROSS=arm-none-eabi-     amalg
#
#---------------------------------------------------------------------------------
# list of directories containing libraries, this must be the top level containing
# include and lib
#---------------------------------------------------------------------------------
LIBDIRS	:= $(CTRULIB) $(PORTLIBS) $(3DS_PORTLIBS)
...
export VPATH	:=	$(foreach dir,$(SOURCES),$(CURDIR)/$(dir)) \
			$(foreach file,$(ADDITIONAL_C_SOURCES),$(CURDIR)/$(dir $(file))) \
			$(foreach file,$(ADDITIONAL_CPP_SOURCES),$(CURDIR)/$(dir $(file))) \
...
CFILES	    :=	$(CFILES) $(foreach file, $(ADDITIONAL_C_SOURCES), $(notdir $(file)))
CPPFILES	:=	$(foreach dir,$(SOURCES),$(notdir $(wildcard $(dir)/*.cpp)))
CPPFILES	:=	$(CPPFILES) $(foreach file, $(ADDITIONAL_CPP_SOURCES), $(notdir $(file)))
SFILES		:=	$(foreach dir,$(SOURCES),$(notdir $(wildcard $(dir)/*.s)))
PICAFILES	:=	$(foreach dir,$(SOURCES),$(notdir $(wildcard $(dir)/*.v.pica)))
PICAFILES	:=	$(PICAFILES) $(foreach dir,$(SOURCES),$(notdir $(wildcard $(dir)/*.f.pica)))
